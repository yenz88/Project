...
\\* By comparing the $X^2$ statistic calculated in Equation \eqref{chisqstat} to the appropiate $\chi^{2}$distribution with $d$ degrees of freedom, we will obtain a p-value for the following null hypothesis.
\vskip3pt $H_0$: All n categories are chosen with equal probability
\vskip3pt $H_a$: All n ncateogires are chosen with unequal probability.
\vskip3pt If the p-value is smaller than a given $\alpha$, it means that the result is not significant and that there is insufficient evidence to reject the null hypothesis that all n categories are chosen with equal probability. However, if the p-value is greater than $\alpha$, we have sufficient evidence to reject the null hypothesis and accept the alternative hypothesis.
\vskip6pt
